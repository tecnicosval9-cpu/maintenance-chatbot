...
def generar_respuesta_con_ia(pregunta, resultados_supabase):
    ...
    if resultados_supabase:
        ...
        for doc in resultados_supabase[:num_fragmentos]:
            ...
            if texto:
                ...
                contexto += f"Fuente: {num_serie} - {tipo}, Página {pagina}\n"
    ...
    # ============================================================
    # PROMPT MEJORADO CON CAPACIDAD DE RESUMEN
    # ============================================================
...
1. Responde SOLO con la información que está en el manual
2. Si la información no está en el manual, di: "No encontré esta información específica en el manual"
3. CITA las fuentes usando el formato: (Máquina, Tipo, Página)
4. NO uses conocimiento general. Solo lo que dice el manual
5. Si la información está en el manual, transcríbela de forma clara y útil

**INSTRUCCIÓN DE RESUMEN (SOLO SI EL USUARIO LO SOLICITA EXPLÍCITAMENTE):**
Si el usuario ha pedido un "resumen", "descripción general" o "puntos principales", organiza tu respuesta en secciones con títulos claros y sintetiza la información más importante de los fragmentos disponibles. Usa emojis para hacer más visual la respuesta (📋, 🎯, 📑, 🔧, 🛡️, ⚙️, etc.)
...
- Si el usuario pide explícitamente un "resumen", "descripción general", "visión general" o "puntos principales":
  * Organiza la información en secciones claras
  * Usa un formato estructurado con títulos y subtítulos
  * Sintetiza los puntos más importantes de los fragmentos disponibles
  * Mantén un tono técnico pero accesible
  * Cita las fuentes principales al final
...
### 🛡️ Seguridad
[Dispositivos y procedimientos críticos]
...
def buscar_en_supabase(pregunta, model, limit=5):
    ...
    # Búsqueda semántica con umbral de similitud
